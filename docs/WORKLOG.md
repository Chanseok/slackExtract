...
#### 5. 메타데이터 자동 등록 (Auto-Registration)
- **문제:** 기존에 다운로드된 파일이나 메타데이터 시스템 도입 전의 파일은 `index.json`에 정보가 없어 분석 결과가 기록되지 않음.
- **해결:** `slack-analyze` 실행 시 인덱스에 없는 채널은 채널명을 ID로 사용하여 자동으로 등록(`EnsureChannel`)하고 분석 정보를 저장하도록 개선.

#### 6. TUI UX 개선 및 그룹핑 기능 (Phase 4)
- **상태바 (Status Bar):** 화면 하단에 전체/표시/선택된 채널 수를 실시간으로 표시하여 가시성 확보.
- **도움말 바 (Help Bar):** 현재 컨텍스트에서 사용 가능한 단축키 안내 강화.
- **메타데이터 시각화:** 채널 목록에 다운로드 완료(`⬇️`) 및 분석 완료(`📝`) 아이콘 표시.
- **수동 그룹핑 (Ad-hoc Grouping):** `g` 키를 눌러 그룹명(폴더명)을 직접 입력하고, 해당 폴더로 다운로드 경로를 지정하는 기능 구현.

#### 7. 메타데이터 백필 (Metadata Backfill)
- **문제:** 메타데이터 시스템 도입 이전에 다운로드된 파일들(`export/project-lg`, `export/archived`)에 대한 정보가 `index.json`에 누락됨.
- **해결:** 일회성 스크립트(`cmd/meta-backfill`)를 작성하여 기존 `.md` 파일들을 스캔하고, 메시지 수와 마지막 수정 시간을 기반으로 메타데이터를 생성하여 `index.json`에 채워 넣음.
...
| 메타데이터 구조 | 단일 인덱스 파일 | 초기 복잡도를 낮추기 위해 `channels/` 하위 폴더 대신 `index.json`에 통합 관리 |
| 메타데이터 백필 | 파일 수정 시간 활용 | 기존 파일의 정확한 다운로드 시점을 알 수 없으므로, 파일 시스템의 수정 시간(ModTime)을 대체값으로 사용 |

### 🎯 다음 작업 (TODO)

1. **검색 기능 강화:** 메타데이터 기반의 오프라인 검색 지원.
2. **설정 관리 시스템:** `.env` 외에 `config.yaml` 등을 도입하여 사용자 설정(기본 경로, 프리셋 등) 관리.
